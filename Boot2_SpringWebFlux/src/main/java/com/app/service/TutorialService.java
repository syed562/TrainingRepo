package com.app.service;

import java.util.Optional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.app.models.Tutorial;
import com.app.repo.TutorialRepo;

import reactor.core.publisher.Flux;
import reactor.core.publisher.Mono;

@Service
public class TutorialService {

	@Autowired
	TutorialRepo tr;
	
	
	public Flux<Tutorial>findAll(){
		return tr.findAll();
	}
	
	public Flux<Tutorial>findByTitle(String title){
		return tr.findByTitle(title);
	}
	
	
	public Mono<Tutorial>findById(int id){
		return tr.findById(id);
	}
	
	public Mono<Tutorial>save(Tutorial t){
		return tr.save(t);
	}
	
	
	public Mono<Tutorial>update(int id,Tutorial t){
		Mono<Tutorial> temp=tr.findById(id);
		return temp.map(Optional::of).defaultIfEmpty(Optional.empty()).flatMap(op->{
			if(op.isPresent()) {
				t.setId(id);
				return tr.save(t);
			}
			return Mono.empty();
		});
	}
	
		public Mono<Void>deleteById(int id){
			return tr.deleteById(id);
		}
		
		
	public Mono<Void>deleteAll(){
		return tr.deleteAll();
	}
	
	public Flux<Tutorial>findByPublished(boolean isPub){
		return tr.findBypublished(isPub);
	}
}
